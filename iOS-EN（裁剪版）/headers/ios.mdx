...
This version adds the `WhiteSDK.prepareWhiteConnectionForAppId` method to preselect the optimal access domain, which can improve the connection speed when users join a whiteboard room for the first time.
...
This version fixes the issue where `drawOnlyPencil` does not work on iOS 17 and above.
...
In multi-window mode, the SDK supports inserting PPT or whiteboard plugins as window apps. Starting from this version, the `WhiteRoom` class introduces the following methods to manage window apps in multi-window mode:
...
- `textColor`: Modifies the color of text in the text tool.
...
- `WKWebView` may cause the failure to open `indexDB` under certain exceptional circumstances.
- After configuring `WhiteWindowParams.fullscreen`, the app may not be displayed in full screen on the second launch.
- After configuring `WhiteWindowParams.fullscreen`, there may be a brief flicker.
...
- `WhiteMemberState` and `WhiteReadonlyMemberState` introduce
    - `textColor`
    - `pencilEraserSize`
- `updateText`
- `WhiteSlideAppParams` introduces
    - `minFPS`
    - `maxFPS`
    - `resolution`
    - `maxResolutionLevel`
